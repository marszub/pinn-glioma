...
class LoadedDiffusionMap(DiffusionMap):
    ...
    def __call__(self, x: Tensor, y: Tensor) -> Tensor:
        ...
        xf = x.floor().long()
        xp = x - xf
        wxf = torch.where(xp <= border, 1, torch.where(
            xp >= 1 - border, 0, 1 - (xp - border) / (1 - 2 * border)))
        wxc = 1 - wxf

        yc = y.ceil().long()
        yf = y.floor().long()
        yp = y - yf
        wyf = torch.where(yp <= border, 1, torch.where(
            yp >= 1 - border, 0, 1 - (yp - border) / (1 - 2 * border)))
        wyc = 1 - wyf
        diffusion = ((wxf * wyf) * self.D[xf, yf] +
                     (wxc * wyf) * self.D[xc, yf] +
                     (wxf * wyc) * self.D[xf, yc] +
                     (wxc * wyc) * self.D[xc, yc])
        return diffusion
