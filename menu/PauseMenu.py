...
class PauseMenu:
    ...
    def run(self) -> None:
        try:
            ...
            while not self.isUnpaused:
                try:
                    command = input(f"PINN:{self.tracker.logDir}> ")
                    self.__dispatch(command)
                except WrongCommandSyntax:
                    print("Wrong syntax in entered command. Type 'help' for more information.")
    ...
    def cd(self, args):
        ...
        if os.path.isdir(path):
            self.tracker.logDir = path
        else:
            print(f"Directory '{path}' does not exist. To create it use 'mkdir'")
    ...
    def mkdir(self, args):
        ...
        if os.path.isfile(path):
            print(f"'{path}' is a file. Delete it or choose different dir name.")
            return
